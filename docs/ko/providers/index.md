...
| Provider | 출력 유형 | 비용 견적 | 미디어 입력 | 계정 요구 사항 | 잔액 | 확인 시점 |
| --- | --- | --- | --- | --- | --- | --- |
| Higgsfield | 이미지와 동영상을 포함해, 노출된 실시간 카탈로그 정의 유형 | 지원되는 생성 종류에 대해 지원 (`$`) | Higgsfield 미디어 ID/UUID; Provider 간 결과는 미디어 URL에서 가져옴 | OAuth를 통한 Higgsfield 계정 | Settings와 캔버스 잔액 칩에 표시 | 마지막 확인: 2026-08 |
...
| Kling | 이미지와 동영상 | 제공되지 않음; `$` 견적 없음 | Provider가 액세스할 수 있는 HTTPS 미디어 URL만 지원 (`inputType: URL`) | OAuth를 통한 Kling 계정 | 멤버십/크레딧 데이터에서 남은 크레딧을 파싱해 표시 | 마지막 확인: 2026-08 |

앱의 실시간 카탈로그가 모델 사용 가능 여부, 매개변수, 지원되는 미디어 포트, 계정별 Kling 모델의 기준입니다. 모델 이름은 이 문서에 일부러 옮기지 않았습니다.

::: warning
Provider 크레딧이 소모됩니다. 카탈로그 조회, 잔액 새로 고침, 지원되는 실행 전 견적은 생성 작업을 제출하지 않습니다. Kling에는 견적 작업이 없습니다.
:::
...
Provider 계정은 서로 독립적입니다. 한 Provider를 연결하거나 연결 해제해도 다른 Provider의 자격 증명을 연결, 연결 해제 또는 교체하지 않습니다.
...
연결을 해제하면 해당 Provider의 로컬 자격 증명 파일을 삭제하고, 캐시된 잔액을 지우며, MCP 세션을 초기화합니다. 다른 Provider에는 영향을 주지 않습니다. 해당 Provider를 다시 사용하려면 **Connect**를 선택하고 OAuth를 다시 완료합니다.
