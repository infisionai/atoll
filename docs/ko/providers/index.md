...
| Magnific | 이미지와 동영상 | 지원되는 모델에 대해 지원; Auto 동영상은 실행 전 견적 없음 | 이미지 생성 식별자; 동영상은 URL 또는 생성 식별자로 된 시작 프레임 하나 사용 | MCP 사용에 Magnific Premium 플랜 필요 | 계정이 `account_balance`에 응답할 수 있을 때 표시; Premium 액세스 필요 | 마지막 확인: 2026-08 |
