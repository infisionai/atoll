...
Higgsfield 계정을 연결하면 실시간 모델 카탈로그를 탐색하고, 공급자 잔액을 확인하고, 선택한 생성 종류가 지원하는 경우 실행 전 `$` 견적을 요청할 수 있습니다.

## 결과

설정을 마치면 Atoll에서 Higgsfield를 독립적인 공급자로 사용할 수 있습니다. 실시간 카탈로그를 불러와 캐시하고, 잔액을 확인하며, 지원되는 생성 노드에 실행 전 `$` 견적을 표시할 수 있습니다.
